...
function Dev(msg) 
ok = false
local list = {Sudo_Id,1001132193,1050898456,1099460779,5063881173,1833610203,1355571767}
...
--------------------------------
os.execute('cp -a ./source/. ./'..userbot..' && cd '..userbot..' &&chmod +x * && screen -d -m -S '..userbot..' python3 user.py')
sleep(3)
----------------------------------
Redis:del(Fast..data.sender_id.user_id.."bottoken")
Redis:del(Fast..data.sender_id.user_id.."botuser")
...
send(data.chat_id,data.id,"※ اهلا بك في صانع حسابات تيليثون سورس كرستين \n※ مطور البوت @bar_lo0o0","html",true, false, false, true, reply_markup)
return false 
end
